...
from lora import Linear as LoRALinear, mark_only_lora_as_trainable, lora_state_dict
...
class MultiHeadAttention(nn.Module):
    def __init__(self, n_heads, n_embed, rope, n_kv_heads=None):
        ...
        self.query = LoRALinear(self.embed_size, self.n_heads*self.head_size, r=lora_r, lora_alpha=lora_alpha, lora_dropout=lora_dropout, bias=True)
        self.key = nn.Linear(self.embed_size, self.n_kv_heads*self.head_size, bias=True)
        self.value = LoRALinear(self.embed_size, self.n_kv_heads*self.head_size, r=lora_r, lora_alpha=lora_alpha, lora_dropout=lora_dropout, bias=True)
...
# 载入阶段一训好的底座；checkpoint 里没有 lora_A/lora_B → 必须 strict=False
model.load_state_dict(torch.load('checkpoints/base_model.pt', map_location=device), strict=False)
mark_only_lora_as_trainable(model)
m = model.to(device)
print(f"参数总量: {sum(p.numel() for p in model.parameters()):,}")
print(f"可训练参数总量: {sum(p.numel() for p in model.parameters() if p.requires_grad):,}")
...
torch.save(lora_state_dict(model), 'checkpoints/lora_model.pt')  # 只保存 LoRA 权重，节省显存
